...
with tab_dashboard:
    ...
    # ROW 2 — SUSTAINABILITY (NEW GAUGE METER) + DISEASE RISK + IRRIGATION
    st.markdown("---")
    ...
    with c_sus:
        ...
        # New Speedometer/Gauge Chart
        fig_gauge = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = sus_data["total"],
            domain = {'x': [0, 1], 'y': [0, 1]},
    ...
    # ROW 5 — AI RECOMMENDATION PANEL & NEW DOWNLOAD BUTTON
    st.markdown("---")
    ...
    if not ai_res:
        ...
        # Download Report Feature (New Feature)
        report_text = f"=== AgriTwin AI - Farm Management Report ===\n"
        report_text += f"Date: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
        report_text += f"Crop: {crop_type} ({growth_stage})\n\n"
        report_text += f"--- SENSOR DATA ---\n"
        report_text += f"Temperature: {temperature}°C\nHumidity: {humidity}%\nSoil Moisture: {soil_moisture}%\nCO2 Level: {co2_level} ppm\n\n"
        report_text += f"--- AI RECOMMENDATIONS ---\n"
        report_text += f"Disease Warning: {ai_res.get('disease_warning')}\n\n"
        report_text += f"Climate Warning: {ai_res.get('climate_warning')}\n\n"
        report_text += f"Irrigation Advice: {ai_res.get('irrigation_advice')}\n\n"
        report_text += f"--- OVERALL ASSESSMENT ---\n{ai_res.get('overall_assessment')}\n"
        
        st.download_button(
            label="📄 Download AI Report (.txt)",
            data=report_text,
            file_name=f"AgriTwin_Report_{crop_type}_{datetime.datetime.now().strftime('%Y%m%d')}.txt",
            mime="text/plain",
            type="secondary",
