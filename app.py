...
Streamlit frontend — NEXT GEN UI (Video BG, 3D Spline, Day/Night, Lottie)
"""
...
# උදේ 6 ත් හවස 6 ත් අතර නම් දවල් (Day) Theme එක, නැත්නම් Night Theme එක
if 6 <= current_hour < 18:
    ...
    """
    # දවල්ට ගැලපෙන කොළ පාට/ඉර එළිය වගේ සජීවී වීඩියෝවක්
    ...
    """
    # රෑට ගැලපෙන අඳුරු සජීවී වීඩියෝවක්
...
.main, section[data-testid="stSidebar"] > div {{ background: transparent !important; }}
...
.metric-tile {{
    ...
    border: 1px solid var(--glass-border); border-radius: 14px; padding: 1rem; text-align: center; backdrop-filter: blur(10px); animation: cardIn 0.5s ease both; transition: transform 0.25s, box-shadow 0.25s;
}}
...
.ai-card {{
    background: linear-gradient(135deg, rgba(124,58,237,0.12), rgba(0,168,255,0.08)); border: 1px solid rgba(124,58,237,0.3); border-radius:16px; padding:1.1rem 1.3rem; backdrop-filter: blur(12px); animation: cardIn 0.7s 0.2s ease both;
}}
...
.section-header {{
    font-family:'Orbitron',sans-serif; font-size:0.95rem; font-weight:600; color:var(--accent) !important;
    letter-spacing:0.1em; text-transform:uppercase; border-bottom: 1px solid var(--glass-border); padding-bottom:0.5rem; margin-bottom:1rem;
}}
...
.stSelectbox div[data-baseweb], .stTextInput input {{ background: rgba(0,0,0,0.3) !important; border: 1px solid var(--glass-border) !important; border-radius:8px !important; color: #E2F8F4 !important;}}
...
with st.sidebar:
    ...
    st.markdown("---")
    # 🧊 Interactive 3D Model in Sidebar
...
# Lottie Animation Loading Container
if run_ai:
    placeholder = st.empty()
    with placeholder.container():
        st.markdown("<h3 style='text-align:center; color:#00DEB4; font-family:Orbitron;'>AI is Analyzing Data...</h3>", unsafe_allow_html=True)
        if lottie_ai_thinking:
            st_lottie(lottie_ai_thinking, height=200, key="thinking")
        else:
            st.spinner("Processing...")
    
    st.session_state["ai_result"] = get_ai_recommendations(params, analysis)
    placeholder.empty() # ඇනිමේෂන් එක අයින් කරනවා රිසල්ට් ආවම
...
with col_h2:
    ...
    st.markdown(
        ...
        f'<span style="color:var(--accent);font-family:Orbitron;font-size:0.7rem;">LIVE</span><br>'
...
with tab_dashboard:
    ...
    with c_sus:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            number={'font': {'color': gc, 'size': 45, 'family': 'Orbitron'}},
            gauge={'axis': {'range': [None, 100], 'tickcolor': "white"}, 'bar': {'color': gc, 'thickness': 0.25},
                   ...
                   'steps': [{'range': [0, 40], 'color': "rgba(239, 68, 68, 0.2)"},
                             ...
                             {'range': [75, 100], 'color': "rgba(0, 222, 180, 0.2)"}]}
        ))
        ...
        html_bars = ""
        metrics = [("Water Efficiency", sus_data["water_efficiency"], "#00A8FF"), ("Energy Efficiency", sus_data["energy_efficiency"], "#7C3AED"),
                   ("Climate Control", sus_data["climate_optimization"], "var(--accent)"), ("Disease Prevention", sus_data["disease_prevention"], "#7FFF00"),
                   ("Yield Potential", sus_data["yield_potential"], "#FFD700")]
        for lbl, val, clr in metrics: html_bars += progress_html(lbl, val, clr)
    ...
    with c_dis:
        ...
        st.markdown(
            ...
            f'<span class="risk-badge" style="background:{dlc}22;color:{dlc};border:1px solid {dlc};">{dis_data["level"].upper()}</span><br>'
            f'<div style="font-family:Orbitron;font-size:2.4rem;color:{dlc};margin:0.4rem 0">{dis_data["overall"]:.1f}<span style="font-size:1rem;">%</span></div>'
            f'<div style="font-size:0.75rem;color:rgba(226,248,244,0.5);">Overall Risk Index</div></div>', unsafe_allow_html=True)
        disease_html = ""
    ...
    with c_irr:
        ...
        st.markdown(
            f'<div class="glass-card" style="text-align:center">'
            f'<span class="status-pill" style="background:{isc}22;color:{isc};border:1px solid {isc};">{irr_data["status"].upper()}</span><br>'
            f'<div style="font-family:Orbitron;font-size:2.2rem;color:{isc};margin:0.4rem 0">{irr_data["urgency"]:.1f}<span style="font-size:1rem;">%</span></div>'
            f'<div style="font-size:0.75rem;color:rgba(226,248,244,0.5);">Urgency Index</div></div>', unsafe_allow_html=True)
        st.markdown(
            f'<div class="glass-card">' + progress_html("Irrigation Urgency", irr_data["urgency"], isc) +
            f'<div style="margin-top:0.8rem;font-size:0.85rem;">📦 Volume Required: <b style="color:{isc}">{irr_data["volume_liters"]:.2f} L/m²</b><br>'
            f'⏰ Next Irrigation: <b style="color:{isc}">{irr_data["next_irrigation_hours"]}h</b><br>'
            f'🌿 ET₀ Rate: <b style="color:{isc}">{irr_data["evapotranspiration"]:.3f} mm/hr</b></div></div>', unsafe_allow_html=True)
    ...
    if not ai_res:
        st.markdown(
            ...
            '<div style="font-family:Orbitron;color:var(--accent);margin:0.5rem 0;">AI Engine Ready</div>'
            f'<div style="color:rgba(226,248,244,0.5);font-size:0.85rem;">Click <b>Run Full AI Analysis</b> in the sidebar.</div>'
...
# =============================================================================
# TAB 2 & 3: CHATBOT & VISION SCANNER (With Auto-Detect API Logic)
# =============================================================================
